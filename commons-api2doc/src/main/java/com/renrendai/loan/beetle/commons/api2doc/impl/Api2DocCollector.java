...
import com.renrendai.loan.beetle.commons.api2doc.annotations.*;
import com.renrendai.loan.beetle.commons.api2doc.domain.*;
...
//            }
        ...
        Api2Doc classApiDoc = clazz.getAnnotation(Api2Doc.class);
        if (classApiDoc != null && classApiDoc.ignore()) {
            // 整个类的文档被忽略。
            if (log.isInfoEnabled()) {
                log.info("@ApiDoc ignore = true, no need to get, " +
                        "beanName = {}", beanName);
            }
            return null;
        }
        ...
        for (MappingMethod mappingMethod : methods) {
            ...
            Api2Doc apiDoc = method.getAnnotation(Api2Doc.class);
            if (classApiDoc == null && apiDoc == null) {
                // 本方法的文档被忽略。
                continue;
            }
            if (apiDoc != null && apiDoc.ignore()) {
                // 本方法的文档被忽略。
                continue;
            }
            ali2DocMethods.add(mappingMethod);
        }
        if (classApiDoc == null && ali2DocMethods.size() == 0) {
        ...
        String id = beanName;
        if (classApiDoc != null && StringUtils.hasText(classApiDoc.value())) {
            id = classApiDoc.value();
        }
        if (classApiDoc != null && StringUtils.hasText(classApiDoc.id())) {
            id = classApiDoc.id();
        }
        folder.setId(id);
        checkId(id);
        ...
        if (classApiDoc != null) {
            folder.setOrder(classApiDoc.order());
        }
        ...
        if (classApiDoc != null && StringUtils.hasText(classApiDoc.name())) {
            name = classApiDoc.name();
        }
        folder.setName(name);
        ...
        for (MappingMethod method : ali2DocMethods) {
            ...
            if (existDoc != null) {
                String msg = "文档id值重复： " + docId + "\n"
                        + "如果方法上没有用  @ApiDoc(id = \"xxx\") 来指定文档id，则重载方法会出现此问题。\n"
                        + "请在重载的方法上用 @ApiDoc(id = \"xxx\") 来指定一个不同的文档id";
                throw new BeanDefinitionStoreException(msg);
            }
    ...
    ApiDocObject getApiDoc(
            ...
            String beanName, Api2Doc classApiDoc,
        ...
        Api2Doc apiDoc = method.getAnnotation(Api2Doc.class);
        if (apiDoc == null && classApiDoc == null) {
            return null;
        }
        ...
        // 获取文档的 id，以 @ApiDoc、方法名 为顺序获取。
        String id = method.getName();
        if (apiDoc != null && StringUtils.hasText(apiDoc.value())) {
            id = apiDoc.value();
        }
        if (apiDoc != null && StringUtils.hasText(apiDoc.id())) {
            id = apiDoc.id();
        }
        doc.setId(id);
        checkId(id);
        ...
        // 获取文档的排序。
        if (apiDoc != null) {
            doc.setOrder(apiDoc.order());
        }

        // 获取文档名称，按 @ApiDoc 、@Mapping、方法名的顺序获取。
        String name = method.getName();
        String mappingName = mappingMethod.getName();
        if (StringUtils.hasText(mappingName)) {
            name = mappingName;
        }
        if (apiDoc != null && StringUtils.hasText(apiDoc.name())) {
            name = apiDoc.name();
        }
        doc.setName(name);
        ...
        // 收集参数信息。
        // modify wudi
        List<ApiParamObject> apiParams = toApiParamsFromAnotation(method);
        if (apiParams == null || apiParams.size() == 0){
            apiParams = toApiParams(method, defaultSeeClass);
        }
...
//        List<ApiParamObject> apiParams = toApiParams(method, defaultSeeClass);
    ...
    public List<ApiParamObject> toApiParamsFromAnotation(Method method){
        List<ApiParamObject> result = new ArrayList<>();
        Set<String> paramIds = new HashSet<>();
        ApiParamComments apiParamComments = method.getAnnotation(ApiParamComments.class);
        if (apiParamComments != null && apiParamComments.value().length > 0){
            for (ApiComment apiComment : apiParamComments.value()) {
                Class elementType = apiComment.seeClass();
                ApiParamObject apiParamObject = new ApiParamObject();
                apiParamObject.setLocation(ApiParamLocation.RequestParam);
                apiParamObject.setName(apiComment.name());
                apiParamObject.setId(apiComment.name());
                ApiCommentUtils.setApiComment(apiComment, String.class, apiParamObject);
                apiParamObject.setSourceType(elementType);
                ApiDataType dataType = convertType(elementType);
                apiParamObject.setDataType(dataType);
                apiParamObject.setJsonType(true);

                if (paramIds.contains(apiParamObject.getId())) {
                    continue;
                }
                paramIds.add(apiParamObject.getId());
